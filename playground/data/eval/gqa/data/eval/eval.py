...
def loadFile(name):
    ...
    # load file chunks if too big
        ...
        for chunk in chunks:
            print(f"=> loading chunk from {chunk}")
...
questions = loadFile(args.questions.format(tier = os.path.basename(args.tier))) # testdev_balanced_questions.json; len=12578
...
predictions = {p["questionId"]: p["prediction"] for p in predictions} # len(predictions)=12578
...
for qid, question in tqdm(questions.items()): # len(questions.items())=12578, qid='201307251',
    # question
    # {'semantic': [{'operation': 'select', 'dependencies': [], 'argument': 'scene'},
    #   {'operation': 'verify weather',
    #    'dependencies': [0],
    #    'argument': 'overcast'}],
    #  'entailed': [],
    #  'equivalent': ['201307251'],
    #  'question': 'Is it overcast?',
    #  'imageId': 'n161313',
    #  'isBalanced': True,
    #  'groups': {'global': None, 'local': '01-weather_overcast'},
    #  'answer': 'no',
    #  'semanticStr': 'select: scene->verify weather: overcast [0]',
    #  'annotations': {'answer': {}, 'question': {}, 'fullAnswer': {}},
    #  'types': {'detailed': 'weatherVerifyC',
    #   'semantic': 'global',
    #   'structural': 'verify'},
